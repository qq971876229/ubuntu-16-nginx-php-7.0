...
function verify_signature($raw_data, $signature, $pub_key_path) {
    ...
    $pub_key_contents = \config::app['pingxx_rsa_public_key'];
...
if ($result === 1) 
{
    ...
    logResult('verification ok');
    // 验证通过
} elseif ($result === 0) {
    http_response_code(400);
    
    
    logResult('verification failed');
...
if ($event['type'] == 'charge.succeeded') 
{
    ...
    $res = logPayModel::paid($order_no,$amount);
...
function logResult($word='') 
{
    ...
    bcl\redis\base::get_rd()->set("log:webhooks:".time(),$word);
