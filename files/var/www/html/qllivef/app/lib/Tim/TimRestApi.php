...
namespace App\Lib\Tim;

use App\Lib\Tim\TimRestInterface;
...
class TimRestApi extends TimRestInterface
{
    ...
    static function create()
    {
        ...
        $TimRestAPI->init(\config::app['live_app_id'], \config::app['live_admin']);

        $TimRestAPI->set_user_sig(TLSSigAPI::createSig(\config::app['live_admin']));
...
//        exit;
    ...
    public function im_push($text_content, $time = 3600 * 24)//消息保存24小时
    {
        ...
        "From_Account"=> \config::app['live_service'],
        "MsgLifeTime"=>$time,
        "MsgBody"=>array(array(
        "MsgType"=>"TIMTextElem",
        "MsgContent"=>array
        ("Text"=>json_encode($text_content)
        ) ))
        );*/
        $desc = $text_content->actionParam;


        $msg_content_elem = array(
            'MsgType' => 'TIMCustomElem',       //自定义消息
            'MsgContent' => array(
                'Data' => json_encode($text_content),
                'Desc' => $desc,
                'Ext' => "ext",
                'Sound' => "sound",
            ),
        ...
        $msg = array(
            ...
            "From_Account" => \config::app['live_service'],
            "MsgLifeTime" => $time,
            "MsgBody" => array($msg_content_elem),
        );
    ...
    //自定义消息	
    public function openim_send_custom_msg($account_id, $receiver, $text_content)
    {
    ...
    //自定义消息,return the vod record
    public function openim_send_custom_msg_vod($account_id, $receiver, $text_content)
    {
    ...
    public function openim_send_msg_text($account_id, $receiver, $text_content)
    {
        ...
        $msg_content_elem = array(
            'MsgType' => 'TIMTextElem',       //文本类型
            'MsgContent' => array(
                'Text' => $text_content->actionParam,                //hello 为文本信息
            ),
        );
        //将创建的元素$msg_content_elem, 加入array $msg_content
        array_push($msg_content, $msg_content_elem);
    ...
    public function querystate($receiver)
    {
    ...
    public function queryattr($receiver)
    {
        ...
        $ret = $this->api("openim", "im_get_attr", $this->identifier, $this->usersig, $req_data);
        $ret = json_decode($ret, true);
    ...
    public function setattr($uid, $key, $value)
    {
...
//        $req_data = json_encode($msg);
        ...
        $ret = $this->api("openim", "im_set_attr", $this->identifier, $this->usersig, $req_data);
        $ret = json_decode($ret, true);
    ...
    public function create_attr_name($name)
    {
...
//        $req_data = json_encode($msg);
        ...
        $ret = $this->api("openim", "im_set_attr_name", $this->identifier, $this->usersig, $req_data);
        $ret = json_decode($ret, true);
...
//            'FaceUrl' => $face_url,
    ...
    public function profile_portrait_get2($account_list, $tag_list)
    {
        ...
        // replace the special chinese char
